...
int* get_dst(int rank, int numProcs, int leader) {
	...
	/* Vectori de parinti */
	MPI_Datatype parent_vec;
    MPI_Type_vector(numProcs, 1, 1, MPI_INT, &parent_vec);
    MPI_Type_commit(&parent_vec);
	...
	if (rank == leader)
		...
		MPI_Recv(vRecv, 1, parent_vec, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &status);
	...
	*/
	for (int i = 0; i < num_neigh; i++) {
		if (v[rank] != neigh[i]) {
			MPI_Send(v, 1, parent_vec, neigh[i], 0, MPI_COMM_WORLD);
		}
	}
	...
	*/
	if (rank != leader) {
		MPI_Send(v, 1, parent_vec, v[rank], 0, MPI_COMM_WORLD);
		MPI_Recv(v, 1, parent_vec, v[rank], 0, MPI_COMM_WORLD, &status);
	}
	...
	*/
	for (int i = 0; i < numProcs; i++) {
		if (v[i] == rank) {
			MPI_Send(v, 1, parent_vec, i, 0, MPI_COMM_WORLD);
		}
	}
...
int leader_chosing(int rank, int nProcesses) {
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		for (int i = 0; i < num_neigh; i++) {
			int neigh_leader;
			MPI_Send(&leader, 1, MPI_INT, neigh[i], k, MPI_COMM_WORLD);
			MPI_Recv(&neigh_leader, 1, MPI_INT, MPI_ANY_SOURCE, k, MPI_COMM_WORLD, &status);
...
int get_number_of_nodes(int rank, int leader) {
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		for (int i = 0; i < num_neigh; i++) {
			MPI_Send(&val, 1, MPI_DOUBLE, neigh[i], k, MPI_COMM_WORLD);
			MPI_Recv(&recvd, 1, MPI_DOUBLE, neigh[i], k, MPI_COMM_WORLD, &status);
			val = (val + recvd) / 2;
		}
	}
...
int ** get_topology(int rank, int nProcesses, int * parents, int leader) {
	MPI_Datatype top_vec;
    MPI_Type_vector(nProcesses, 1, 1, MPI_INT, &top_vec);
    MPI_Type_commit(&top_vec);
	...
	for (int i = 0; i < nProcesses; i++) {
		if (rank == parents[i]) {
			for (int j = 0; j < nProcesses; j++) {
				MPI_Recv(vTopology[j], 1, top_vec, i, 0, MPI_COMM_WORLD, &status);
	...
	for (int i = 0; i < nProcesses; i++) {
		if (parents[rank] != -1) {
			MPI_Send(topology[i], 1, top_vec, parents[rank], 0, MPI_COMM_WORLD);
		}
	}
	...
	if (rank != leader) {
		for (int i = 0; i < nProcesses; i++) {
			MPI_Recv(topology[i], 1, top_vec, parents[rank], 0, MPI_COMM_WORLD, &status);
		}
	}
	...
	for (int i = 0; i < num_neigh; i++) {
		if (neigh[i] != parents[rank]) {
			for (int j = 0; j < nProcesses; j++) {
				MPI_Send(topology[j], 1, top_vec, neigh[i], 0, MPI_COMM_WORLD);
			}
		}
	}
